...
BEGIN
    IF EXISTS (
        ...
        WHERE table_schema = 'financeiro' AND table_name = 'cliente_financeiro' AND column_name = 'cpf'
    ) THEN
        UPDATE cliente_financeiro SET cpf_encrypted = cpf WHERE cpf_encrypted IS NULL AND cpf IS NOT NULL;
    END IF;
    IF EXISTS (
        SELECT 1 FROM information_schema.columns
        WHERE table_schema = 'financeiro' AND table_name = 'cliente_financeiro' AND column_name = 'email'
    ) THEN
        UPDATE cliente_financeiro SET email_encrypted = email WHERE email_encrypted IS NULL AND email IS NOT NULL;
    END IF;
    IF EXISTS (
        SELECT 1 FROM information_schema.columns
        WHERE table_schema = 'financeiro' AND table_name = 'cliente_financeiro' AND column_name = 'telefone'
    ) THEN
...
BEGIN
    IF EXISTS (
        SELECT 1 FROM information_schema.columns
        WHERE table_schema = 'financeiro' AND table_name = 'pre_nfse' AND column_name = 'dados_tomador_json'
    ) THEN
...
BEGIN
    IF EXISTS (
        SELECT 1 FROM information_schema.columns
        WHERE table_schema = 'financeiro' AND table_name = 'envio_lembrete_cobranca' AND column_name = 'destino'
    ) THEN
