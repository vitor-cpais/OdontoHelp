import {
  ...
  TableHead, TableRow, MenuItem, TextField,
  Snackbar, Alert,
} from '@mui/material';
import { AddOutlined, ExpandMoreOutlined, AssignmentOutlined } from '@mui/icons-material';
import { useState } from 'react';
import { usePlanoUnico, useAtualizarStatusItem } from './usePlanoTratamento';
import { getApiErrorMessage } from '../../shared/lib/axios';
import PlanoTratamentoDrawer from './PlanoTratamentoDrawer';
import { STATUS_ITEM_PLANO_LABELS, STATUS_ITEM_PLANO_COLORS } from './types';
    ...
    return (
      <Chip
        ...
        sx={{
          ...
          border: '1px solid', borderColor: colors.border, fontWeight: 500,
        }}
      />
    );
  }
...
function PlanoCard({
  ...
  const emAndamento  = plano.itens.filter((i) => i.status === 'EM_ANDAMENTO').length;
  ...
  return (
    ...
    >
      <AccordionSummary expandIcon={<ExpandMoreOutlined />} sx={{ borderRadius: 2, minHeight: 56 }}>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, width: '100%', pr: 1 }}>
          ...
          <Box sx={{ display: 'flex', gap: 0.75 }}>
            ...
            {emAndamento > 0 && (
              <Chip label={`${emAndamento} em andamento`} size="small"
                sx={{ fontSize: '0.68rem', height: 20, backgroundColor: '#E6F1FB', color: '#185FA5', border: '1px solid #B5D4F4' }} />
            )}
      ...
      <AccordionDetails sx={{ pt: 0, px: 0 }}>
        {plano.observacoes && (
          <Box sx={{ px: 2, pb: 1.5 }}>
            <Typography variant="caption" color="text.secondary">{plano.observacoes}</Typography>
          </Box>
        )}
        <TableContainer>
          <Table size="small">
                ...
                .map((item) => {
                  ...
                  return (
                    <TableRow key={item.id} hover
                      sx={{ opacity: item.status === 'CANCELADO' ? 0.45 : 1 }}>
                      <TableCell>
...
export default function PlanoTratamentoTab({ pacienteId, onNovo, dentistaId, useDialogForDrawer }: Props) {
  const [drawerOpen, setDrawerOpen] = useState(false);
  const [toast, setToast] = useState<{ open: boolean; msg: string; severity: 'success' | 'error' }>({
    open: false, msg: '', severity: 'success',
  });

  const { data: plano, isLoading } = usePlanoUnico(pacienteId);
      ...
      {isLoading ? (
        ...
        <Box sx={{ py: 6, textAlign: 'center' }}>
          ...
          <Typography variant="body2" color="text.disabled">
            Nenhum item no plano de tratamento
          </Typography>
        </Box>
      ) : (
        planos.map((plano) => (
          <PlanoCard
            key={plano.id} plano={plano}
